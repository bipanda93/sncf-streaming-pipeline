...
```mermaid
erDiagram
    GOLD_DISRUPTION_CONTEXT ||--o{ GOLD_DISRUPTION_BY_REGION : "1 perturbation -> N régions"
    GOLD_DISRUPTION_CONTEXT ||--o{ GOLD_DISRUPTION_BY_STATION : "1 perturbation -> N gares"
    GOLD_DISRUPTION_CONTEXT ||--|| GOLD_REALTIME_ALERTS : "1-1"
    GOLD_DISRUPTION_CONTEXT ||--|| SNCF_DISRUPTIONS : "1-1 (inactive, Silver)"
    ...
    SNCF_DISRUPTIONS {
        string disruption_id PK
        string status "active / past / future"
        datetime period_begin
        date Jour
    }
    ...
    GOLD_PUNCTUALITY_TRENDS {
        string dataset_key
        string scope_type "region / liaison / axe / national"
        string scope_value
